...
export default {
  ...
  methods: {
      ...
      if (
        ...
        for (let i = 0; i < possiblePrefixes.length; i++) {
          ...
          if (prev.startsWith(ins)) {
            value =
              value.slice(0, selectionEnd) + ins + value.slice(selectionEnd)
    ...
    isNewLine() {
      const { selectionStart } = this.$refs.input
    ...
    handleAddLink(linkOptions) {
      let { title, link } = linkOptions
      if (title === '') {
        title = link
      }
